...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for queeni in qi.domain():
            for queenj in qj.domain():
                if abs(queeni - queenj) != abs(self.i - self.j):
                    if queeni != queenj:
                        satisfyingAssignments.append([queeni, queenj])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        return self._lb <= len(assignments) >= self._ub
