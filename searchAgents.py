...
def cornersHeuristic(state, problem):
    ...
    while len(unvisited) != 0:
        position = findMinDistance(current, unvisited)
        current = position[0]
        cost += position[1]
...
def findMinDistance (start, corners):
    distance = 99999999999
    new_position = 0

    for corner in corners:
        if distanceBetween(start, corner) < distance:
            distance = distanceBetween(start, corner)
            new_position = corner
...
def foodHeuristic(state, problem):
    ...
    for food in grid.asList():
        dist = distanceBetween(position, food)
        if dist > far:
            far = dist
        for point in grid.asList():
            d = distanceBetween(food, point)
            if d > between:
                between = d
    return max(between, far)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        for dot in self.food.asList():
            distance = distanceBetween(state, dot)
            priority.push(dot, distance)
        dot = priority.pop()
        return dot == state
